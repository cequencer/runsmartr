...
def plot_sf(m, highways_boundary, lng0, lat0):
    ...
    map_highways = pd.DataFrame({
        'poly': list(highways_boundary)
    })
...
#     route_points = MultiPoint(list(map_points.values))
    ...
    map_highways['patches'] = map_highways['poly'].map(lambda x: PolygonPatch(
        x, fc='#aa0000', ec='#440000', lw=.5, alpha=.9, zorder=3))
...
#               alpha=0.9, zorder=3)
    ...
    ax.add_collection(PatchCollection(map_highways['patches'].values,
                                      match_original=True))
...
#     location = geocoder.geocode('presidio san francisco')
...
#     # Get roads/trails buffered by 20 m
#     cur.execute("""
# SELECT ST_AsText(the_geom) FROM highways_buffered;""")
#     print time.time()-t
#     highways_boundary = shapely.ops.transform(m, shapely.wkt.loads(cur.fetchone()[0]))
#     print time.time()-t

#     # Store buffered roads/trails as new geography
#     print 'Adding geometry to database ...'
#     cur.execute("""
# CREATE TABLE highways_buffered AS
# (SELECT ST_Buffer(ST_Collect(highway)::geography, 20)::geometry AS the_geom FROM
# (SELECT linestring AS highway FROM ways, neighborhoods
# WHERE tags::hstore ? 'highway' AND ST_Intersects(linestring, polygon)) AS foo);""")
#     conn.commit()
...
#     route_coords = np.array([ el['coordinates'] for el in route_coords ])
    ...
    plot_sf(m, highways_boundary, lng, lat)
