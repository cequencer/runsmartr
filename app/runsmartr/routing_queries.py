    ...
    def rand_rnode_within_m(self, rnode, m):
        query = ("""
            ...
            ORDER BY search_point <-> point LIMIT 1;"""
                 % ((m-m/2.)*rnd()/89012., (m-m/2.)*rnd()/110978.,
                    self.rnodes_table, rnode, self.rnodes_table))
    ...
    def rand_rnode_within_threshold(self, rnode, threshold):
        query = ("""
            ...
            ORDER BY RANDOM() LIMIT 1;"""
                 % (self.rnodes_table, self.rnodes_table,
                    rnode, threshold))
        self.db_cur.execute(query)
        return self.db_cur.fetchall()[0][0]
    ...
    def distance(self, n1, n2):
        ...
        query = ("""
            SELECT ST_Distance(point_1::geography, point_2::geography) FROM
                ...
                (SELECT point AS point_2 FROM %s WHERE id = '%d') AS rnode_2;"""
                 % (self.rnodes_table, n1, self.rnodes_table, n2)
        self.db_cur.execute(query)
        return self.db_cur.fetchall()[0][0]
